/**
 * Audio handling utility for Ananya Live API Voice stream.
 * Handles:
...
export class AnanyaAudioSession {
  ...
  private onToolCall: (name: string, args: any, callback: (result: any) => void, handledByExtension?: boolean) => void;
  ...
  constructor(handlers: {
    ...
    onToolCall: (name: string, args: any, callback: (result: any) => void, handledByExtension?: boolean) => void;
  ...
  public async connect() {
    ...
    try {
      ...
      this.ws.onopen = async () => {
        console.log("[Ananya] Connected to server side WS bridge");
        try {
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          if (data.type === "status") {
            console.log("[Ananya WS Status]:", data.status);
          ...
          // Handle interruption signal (e.g. user talked over Ananya)
          ...
          if (data.type === "turnComplete") {
            // Once Ananya completes speaking, change visual state back to listening
            setTimeout(() => {
          ...
          if (data.type === "toolCall") {
            const { callId, name, args, handledByExtension } = data;
            this.onToolCall(name, args, (result) => {
              // Only send back execution result to server bridge if not handled by extension
              if (!handledByExtension && this.ws && this.ws.readyState === WebSocket.OPEN) {
...
// Start "speaking" exactly when the first audio chunk starts
if (this.activeSources.length === 0) {
  const delay = Math.max(
    0,
    (scheduledStartTime - currentTime) * 1000
  );
