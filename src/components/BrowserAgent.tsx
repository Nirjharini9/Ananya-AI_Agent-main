...
export const BrowserAgent: React.FC<BrowserAgentProps> = ({
  ...
  const lastWindowOpenRef = useRef<number>(0);
  ...
  useEffect(() => {
    const handleNavigationMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === "NAVIGATE" && event.data.url) {
        console.log("[Ananya Browser] Same-origin child iframe navigated to:", event.data.url);
  ...
  useEffect(() => {
    ...
    console.log(`[Ananya Browser Hub] Automated Voice Trigger: ${type}`, args);

    const runVoiceAutomation = async () => {
      let activeLocalConnected = isLocalConnected;
      if (!activeLocalConnected) {
        try {
          const checkRes = await fetch("http://localhost:3001/api/status", { method: "GET" });
          if (checkRes.ok) activeLocalConnected = true;
        } catch(e) {}
      }
      ...
      if (activeLocalConnected) {
        try {
          ...
          callback({ result: data.result || `Forwarded ${type} to Local Agent.` });
        } catch (err: any) {
          console.error("[Ananya Browser] Local agent execution failed.", err);
          callback({ result: `Error from Local Agent: ${err.message}` });
        }
        return; // Always return if local agent is targeted. Do NOT fall through.
      }
    ...
    if (activeTab) {
      ...
      try {
        ...
        if (iframe && iframe.contentDocument) {
          ...
          if (bodyTxt.includes("Ananya Web Proxy Error") || bodyTxt.includes("Failed loading remote website")) {
  ...
  return (
    <div
      id="ananya-playwright-automation-hud"
    ...
    >
      <div className="relative w-full max-w-5xl h-[88vh] flex flex-col rounded-3xl border border-[var(--border-color)] bg-[var(--bg-card)] shadow-[0_2px_6px_rgba(0,0,0,0.15)] overflow-hidden">
          ...
          <div className="flex items-center gap-3 pb-2.5">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[10px] font-mono tracking-wider font-extrabold cursor-pointer transition ${
                showDebugPanel 
                  ? "bg-amber-900/20 border-amber-500/30 text-amber-400 shadow-[0_2px_6px_rgba(0,0,0,0.15)]" 
                  : "bg-white/5 border-white/5 text-slate-400 hover:text-white"
              }`}
            >
              <Terminal size={12} className={showDebugPanel ? "text-amber-400" : ""} />
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-[#07070c] relative">
            ...
            {activeTab?.url === "about:blank" ? (
              ...
              <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center justify-center space-y-8 select-none text-center scrollbar-none">
                ...
                >
                  <div className="mx-auto w-14 h-14 rounded-2xl bg-[var(--bg-surface)] border border-[var(--border-color)] flex items-center justify-center shadow-[0_2px_6px_rgba(0,0,0,0.15)] col-span-1">
                    <Globe size={26} className="text-[var(--accent-primary)]" />
                  </div>
                  <div>
                ...
                <div className="p-3 border border-white/5 bg-slate-950/50 rounded-2xl max-w-md w-full">
                  <p className="text-[9px] font-mono text-slate-500 leading-normal uppercase">
                    🌸 COMPANION TIP: Voice command Ananya to trigger any automation: <br />
              ...
              <div className="flex-1 w-full h-full p-8 flex flex-col items-center justify-center bg-slate-950/80 text-center relative overflow-y-auto scrollbar-none">
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  className="max-w-xl p-8 rounded-3xl border border-[var(--border-color)] bg-[var(--bg-surface)] backdrop-blur-md space-y-6 shadow-[0_2px_6px_rgba(0,0,0,0.15)]"
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-sans text-slate-400 leading-relaxed max-w-md mx-auto">
                      Ananya automatically detected secure sandbox constraints for <span className="font-mono text-indigo-300 font-semibold">{getCleanTitleFromUrl(activeTab?.url || "")}</span>. 
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-sans text-slate-400 leading-relaxed max-w-sm mx-auto font-medium">
                      Ananya was unable to load <span className="font-mono text-indigo-300 font-semibold">{getCleanTitleFromUrl(activeTab?.url || "")}</span> securely. This website may be offline, spelled incorrectly, or blocking automated server access.
                    </p>
                  </div>
              ...
              <div className="flex-1 w-full h-full bg-slate-950 flex flex-col overflow-hidden relative">
                ...
                {ytSearchLoading ? (
                  <div className="flex-1 flex flex-col items-center justify-center space-y-3.5">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-red-500 font-extrabold">Parsing Video Matrix Stream...</span>
                  </div>
                ) : ytSearchError ? (
              ...
              <div className="flex-1 w-full h-full relative overflow-hidden">
                ...
                {activeTab?.isLoading && (
                  <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center space-y-4">
                    ...
                    <span className="font-mono text-[10px] text-[var(--accent-primary)] tracking-[0.25em] font-bold uppercase">
                  ...
                  {!isLocalConnected ? (
                    ...
                    <div className="flex-1 overflow-y-auto space-y-5 pr-1 select-text">
                      <div className="p-3.5 rounded-xl border border-indigo-500/10 bg-indigo-505/10 text-[11px] font-sans leading-normal text-indigo-300">
                        ...
                        <span>If you prefer to command a real, headed Chrome browser on your desktop computer, easily launch Ananya's local client script!</span>
                      </div>
                    ...
                    <div className="flex-1 overflow-y-auto font-mono text-[10px] space-y-2 pr-1 select-text scrollbar-thin">
                      {localLogs.length === 0 ? (
                        <div className="text-slate-600 italic">No operations recorded yet. Speaking with Ananya will generate real terminal logs.</div>
                      ) : (
                        localLogs.map((log) => (
                          <div
                            key={log.id}
              ...
              <div className="p-4 grid grid-cols-1 md:grid-cols-4 gap-4 font-mono text-[10px] tracking-wide text-slate-400 leading-normal max-h-48 overflow-y-auto select-text scrollbar-thin">
                ...
                <div className="md:col-span-4 flex items-center justify-between border-b border-white/5 pb-1.5 mb-1 text-[11px] font-extrabold uppercase text-amber-400">
                  <div className="flex items-center gap-1.5">
                    <Terminal size={13} />
                    <span>Active Ananya Browser Diagnostics console</span>
                  </div>
