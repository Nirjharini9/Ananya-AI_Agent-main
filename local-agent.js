/**
 * Ananya Playwright Local Agent Server
 * Run this locally on your machine to grant Ananya real control over your browser!
 * 
 * Setup instructions:
 ...
 * 
 * This server binds to port 3001 on localhost, permitting Ananya's web portal to issue
  ...
  try {
    ...
    switch (type) {
      ...
      case "browserSearch": {
        ...
        const isYoutubeIntent = currentUrl.includes("youtube.com") || query.toLowerCase().includes("youtube") || query.toLowerCase().includes("video");

        if (isYoutubeIntent) {
          const cleanQ = query.replace(/youtube|video/gi, "").trim();
          const destUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(cleanQ || query)}`;
          logAndBroadcast(`Navigating directly to YouTube results: ${destUrl}`, "info");
          ...
          logAndBroadcast(`Page is blank, opening Google search for: "${query}"`, "info");
          await page.goto(`https://www.google.com/search?q=${encodeURIComponent(query)}`, { waitUntil: "domcontentloaded", timeout: 60000 });
        } else {
          // General input heuristic search fallback
      ...
      case "browserClick": {
        ...
        if (selector.startsWith("video-") && selector !== "video-title") {
      ...
      default:
        throw new Error(`Directive '${type}' not recognized by Ananya's local Playwright engine.`);
    }
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`\n======================================================`);
  console.log(`🚀 Ananya Playwright Local Agent Server Running!`);
